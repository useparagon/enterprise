...
**Public assets:** the CDN bucket is private in `infra`. Set `CLOUD_STORAGE_PUBLIC_URL` and `CDN_PUBLIC_URL` to the HTTPS base URL of the service that proxies public reads (for example your MinIO ingress or another app edge), not the raw blob host.
